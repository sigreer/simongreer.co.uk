...
To enable automated deployments, you need to create a deploy hook. This opens an HTTP endpoint that Cloudflare listens on. To complete the configuration, add this URL to the Github Actions workflow as a trigger.

<div class="flex flex-row">
    <div class="not-prose flex">
    <LightBox src={cloudflarePagesDeployHook}
    alt="Cloudflare deploy hook"
    width={300}
    class="my-4"
    caption="Cloudflare Pages deploy hook"
    />
    </div>
    <div class="not-prose flex">
    <LightBox src={githubActionsWorkflowTrigger}
    alt="Cloudflare deploy hook"
    width={300}
    class="my-4"
    caption="GitHub Actions workflow trigger configuration"
    />
    </div>
</div>
