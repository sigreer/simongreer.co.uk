...
import cloudflarePagesDashboard from '@images/galleries/astro-on-cloudflare/cloudflare-pages-dashboard.png'; 
...
I'll not cover creating an Astro site or initialising its Git repo. I'm assuming that a site already exists locally and is linked to a Github respositor. With that in mind, this is all you need to do to get up and running with Cloudflare Pages:

1. Create a Pages project on Cloudflare
2. Authenticate Cloudflare's access to Github
3. Configure build preferences and set the workflow trigger
4. Push Astro code to repository
