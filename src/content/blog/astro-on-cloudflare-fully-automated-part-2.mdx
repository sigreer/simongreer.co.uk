...
In the last step we added a config file path for Wrangler to the Cloudflare adapter's config. Let's create that and add the necessary configurations to it.
...
<LightBox src={mailtrapApiDetails}
  ...
  width={300}
  class="my-4"
  caption="Mailtrap domain API details"
/>
</div>
...
Once done you should have a Cloudflare Worker deployed using Wrangler that 